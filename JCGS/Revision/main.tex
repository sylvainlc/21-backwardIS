...
a pseudo marginal SMC approach, proposing a workaround, the backward importance sampling, to the computationally intensive accept-rejection approach of \cite{gloaguen2018online}.
In Section \ref{sec:simu:RNN}, we highlighted the performance of this estimator in the context of hidden state estimation for RNN and showed that we perform better than the classical path-space smoother when the dimension of the hidden state increases. 
In Section \ref{sec:simu:SINE}, an empirical study using a partially observed diffusion process suggests that the bias of this estimator can quickly become negligible with enough Monte Carlo samples, and is less computationnaly intensive than the state-of-the art unbiased estimator.
In Section \ref{sec:simu:SINE}, conjugate to a novel unbiased estimate of the gradient of log transition density for diffusion process, pseudo marginal SMC can be used to perform online estimation in partially observed diffusion process.
