...
In this paper, we propose efficient algorithm to compute \textit{expectations of smoothing additive functionals}. 
...
\end{equation}
where $\addf{k}$ is a functional from $\rset^{d} \times \rset^{d}$ to $\rset^{d'}$.
Such expectations are the keystones for many common inference problem in state space models.

\begin{example}[State estimation]
\label{ex:state:tracking}
Suppose that the model parameter $\theta$ is known, a common objective is to recover the underlying signal $X_{k^*}$ for some index $0\leqslant k^* \leqslant n$ given the observations $Y_{0:n}$. A standard estimator is
$$\pE\left[X_{k^*}\vert Y_{0:n}\right],$$
which is a particular instance of our problem with $\addf{k}(x_k, x_{k+1}) = x_k$ if $k = k^*$ and 0 otherwise. 
\end{example}

\begin{example}[EM algorithm]
\label{ex:em:algorithm}
In the usual case when $\theta$ is unknown, the maximum likelihood estimator is
$$
\widehat \parvec = \mathrm{argmax}_{\parvec\in\parspace}\eqsp\llh{n,\parvec}(Y_{0:n})\eqsp,
$$
where  Expectation Maximization based algorithms \cite{dempster1977maximum}  are appealing solutions to obtain an estimator of $\hat \parvec$.
...
Property \eqref{eq:property:filt:smooth} essentialy tells us that the targetted smoothing expectation is the filtering expectation of a well chosen statistic, while property \eqref{eq:property:backward} provides  a recursion to compute this statistic. 
...
$$
Then, using \eqref{eq:property:filt:smooth}, $\post{0:n \mid n}[\af{0:n}]$ is estimated by  
\[
\post{0:n\mid n}^{\N} [\af{0:n}] \eqdef \sum_{i=1}^{\N}\frac{\ewght{n}{i}}{\sumwght{n}}\tstat[i]{n}\eqsp.
\]
As explained in \cite{gloaguen2021pseudo}, this recursive update requires to produce samples $\bi{k+1}{i}{j}$ distributed according to the marginal  distribution of $\overline \upsilon_k^i$, referred to as the backward kernel.  The theoretical guarantees established in \cite{gloaguen2021pseudo}, i.e. exponential concentration and asymptotic normality of $\post{0:n\mid n}^{\N} [\af{0:n}]$, assume that sampling from $\overline \upsilon_k^i$ is possible. In practice, this requires computationally intensive sampling procedures and the only proposed practical solution can be used in very restrictive situations.  
This solution was proposed in \cite{gloaguen2018online}, where the authors assumed that  almost surely, for all $x,x'$, $\hatqg{k}\langle \zeta\rangle(x, x')\geqslant 0$, and that, for all  $0\leqslant k\leqslant n$ and $0\leqslant i\leqslant N$, there exists an upper bound $\hkup^i_k$  such that
\begin{equation}
...
Recurrent Neural Networks (RNN) were first introduced in \cite{Mozer1989AFB} to model time series using a hidden context state  which is appealing to describe short time dependencies. Although they are lackluster in problems requiring long term memory due to vanishing and exploding gradients \cite{Bengio1994LearningLD} and they have been extended by Long Short Term Memory  \cite{Hochreiter1997LongSM} and Gated Recurrent Unit (GRU) \cite{Cho2014LearningPR}, they define robust many to many backbone architectures which are widely used in practice \cite{mikolov2010recurrent, sutskever2011generating, sutskever2014sequence}.
...
Such general state space model can be extended straightforwardly to multi-layer RNN architectures by injecting noise on each hidden layer, and to RNN variants such as LSTM and GRU. 

From this general state space model based on a stochastic RNN, in the context of the \textit{state estimation} problem, we can use particle smoothing algorithms to estimate the smoothing expectation $\mathbb{E}[X_k|Y_{0:n}]$, for $k \in \{0,...,n\}$. We will call $\mathbb{\widehat{E}}[X_k|Y_{0:n}]$ such estimation. 


Table~\ref{table:RNN_exp} presents the results for a backward IS smoother versus a Poor Man's smoother when estimating $\mathbb{E}[X_k|Y_{0:n}]$.
To generate a sequence of states and observations $(X_{0:n},Y_{0:n})$ from the stochastic RNN, we first trained a deterministic one-layer RNN on 20,000 samples of the weather dataset\footnote{https://www.bgc-jena.mpg.de/wetter/}, and inject noise post-training on the trained RNN.  We consider diagonal covariance matrices $\Sigma$, $Q$ and $R$, with the same variance along all dimensions equal to 0.1. 

The table displays two quantities, when performing 100 runs for each smoothing algorithm: (i) the Mean Squared error $\mathbb{E}[\|X_0 - \mathbb{\widehat{E}}[X_0|Y_{0:n}]\|^2]$ for the conditional expectation of $X_0$ and (ii) the average Mean Squared Error $\mathbb{E}[\frac{1}{n+1} \sum_{k=0}^{n}\|X_k - \mathbb{\widehat{E}}[X_k|Y_{0:n}]\|^2]$ for all conditional expectations of $X_k$, $k \in \{0,..,n\}$.
The experiments were done for RNNs of hidden dimension 32 and 64, and for a sequence of observations with a total length of 200 time steps: the table then presents the evaluation of the above quantities when doing online estimation for such sequence, for $n=49$ (sequence truncated at 50 observations), $n=99$ (sequence truncated at 100 observations, and $n=199$ (full sequence). 
For the backward IS smoother, we use $N=1000$ particles for the bootstrap filter and $\K=32$ backward samples (see figure~\ref{fig:sine:timeandbias:N:vary} for the choice of $\K$ from the number of particles $N$). We use for the Poor Man's smoother a number of particles $N=3,000$, giving a similar computational cost than the backward IS smoother. One interesting aspect of applying the backward IS smoother (BIS) on neural network architectures is the parallelization abilities of such algorithm: when sampling from neural network's observation models, the loop over the backward samples in the backward step of the BIS is easily parallelizable, while the Poor Man's smoother requires to keep at time step the full past trajectories of the particles. 

\begin{table}
\caption{$\mathbb{E}[\|X_0 - \mathbb{\widehat{E}}[X_0|Y_{0:n}]\|^2]$ and $\mathbb{E}[\frac{1}{n+1} \sum_{k=0}^{n}\|X_k - \mathbb{\widehat{E}}[X_k|Y_{0:n}]\|^2]$ for the Poor man's smoother (PMS) and the Backward IS smoother, for states $\{X_k\}_{k=0}^n$ and observations $\{X_k\}_{k=0}^n$ generated with stochastic RNNs of dimension 32 and 64. We consider an online estimation of a sequence of 200 observations, truncated at timestep $n=49$ (50 timesteps), $n=99$ (100 timesteps) and the full sequence ($n=199$).}
\label{table:RNN_exp}
\centering 
\begin{tabular}{c|cc}
quantity & PMS & Backward IS \\\toprule
\hline
RNN dim = 32 & & \\
\hline
$\|X_0 - \mathbb{\widehat{E}}[X_0|Y_{0:199}]\|^2$ & 0.2147 & 0.1997 \\
$\frac{1}{200} \sum_{k=0}^{199} \|X_k - \mathbb{\widehat{E}}[X_k|Y_{0:199}]\|^2$ & 0.2551 & 0.2056 \\
\hline
$\|X_0 - \mathbb{\widehat{E}}[X_0|Y_{0:99}]\|^2$ & 0.2135 & 0.1997 \\
$\frac{1}{100} \sum_{k=0}^{99} \|X_k - \mathbb{\widehat{E}}[X_k|Y_{0:99}]\|^2$ & 0.2531 & 0.2189 \\
\hline
$\|X_0 - \mathbb{\widehat{E}}[X_0|Y_{0:49}]\|^2$ & 0.2018 & 0.1997 \\
$\frac{1}{50} \sum_{k=0}^{49} \|X_k - \mathbb{\widehat{E}}[X_k|Y_{0:49}]\|^2$ & 0.2307 & 0.2147 \\
\hline
\midrule
RNN dim = 64 & & \\
\hline
...
The results show that the backward IS smoother outperform the Poor Man's, both when estimating 
%the conditional expectation of $X_0$ 
$\mathbb{E}[X_0|Y_{0:n}]$, and the average over all time steps $k$ of $\mathbb{E}[X_k|Y_{0:n}]$. This is also illustrated in the figure~\ref{fig:RNN:mseperXk}, displaying for the stochastic RNN of dimension $64$ the Mean Square Error (MSE) over 100 runs $\mathbb{E}[\|X_k - \mathbb{\widehat{E}}[X_k|Y_{0:199}]\|^2]$, for $k \in \{0,...,199\}$: the backward IS smoother has a smaller MSE than the Poor Man's smoother, as soon as $k \leq 150$.  
Moreover, the table also shows that while the backward IS's estimation $\mathbb{\widehat{E}}[X_0|Y_{0:n}]$ tends to stay stable for all given $n$ (49,99, and 199), the Poor Man's estimation is less accurate for a longer sequence of observations, with a MSE $\mathbb{E}[\|X_0 - \mathbb{\widehat{E}}[X_0|Y_{0:n}]\|^2]$ increasing as $n$ increases.

\begin{figure}[h]
\begin{center}
    %\includegraphics[scale = .3]{RNN_dim64_mse_all_Xk.pdf}
    \includegraphics[width=\textwidth, trim = 1cm 2cm 1cm 2cm, clip]{RNN_dim64_mse_all_Xk.pdf}
\end{center}
    \caption{Plot of $\mathbb{E}[\|X_k - \mathbb{\widehat{E}}[X_k|Y_{0:199}]\|^2]$ for $k \in \{0,...,199\}$ for 100 runs of the Backward IS and Poor Man smoothers.}
...
$$
This simple model has no explicit transition density, however, a General Poisson estimator which satisfies \eqref{eq:AR:bound} can be computed by simulating Brownian bridges, see \cite{beskos2006exact} and additional details in Appendix~\ref{sec:filter:SDE}. 
In this particular case, the backward importance sampling technique proposed in this paper can be compared to the usual acceptance-rejection. 
...
As explained in Section~\ref{sec:model}, it is then required to estimate iteratively, from an initial guess $\theta_0$, the conditional expectation given in equation \eqref{eq:EM:E:step}.
