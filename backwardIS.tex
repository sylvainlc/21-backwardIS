...
We focus here in the context of \textit{state space models}. 
Formally, let $(X_k)_{k\geqslant 0}$ be a Markov chain on $\rset^d$ such that the distribution of $X_0$ has density $\chi$ with respect to the Lebesgue measure and for all $0\leqslant k \leqslant n-1$, the conditional distribution of $X_{k+1} $ given $X_{0:k}$ has density $\hd{k+1;\parvec}(X_{k},\cdot)$ ($a_{u:v}$ is a short-hand notation for $(a_u,\ldots,a_v)$). 
...
In this context, for any pair of indexes $0\leqslant\leqslant k_1 \leqslant k_2 \leqslant n$, we define the \textit{joint smoothing distribution} as the law of $X_{k_1:k_2}\vert Y_{0:n}$. 


In this paper, we extend and intensively explore a generic and efficient algorithm to compute \textit{expectations of smoothing additive functionals}. 
...
where $\addf{k}$ is a functional from $\rset^{d \times 2}$ $\rset^{d'}$.
If this problem can seem rather abstract at first, it is actually the keystone for many common inference problem in state space models.

\begin{example}[state tracking]
\label{ex:state:tracking}
Suppose that the model parameter $\theta$ is known, a common objective aims at recovering the underlying signal $X_{k^*}$ for some index $0\leqslant k^* \leqslant n$ given the observations $Y_{0:n}$. The natural Bayesian estimator is given by:
...
In the usual case when $\theta$ is unknown, a classical estimator is given by:
$$
\widehat \parvec = \mathrm{argmax}_{\parvec\in\parspace}\eqsp\llh{n}(\parvec)\eqsp,
$$
where $\llh{n}(\parvec)$ is the likelihood of the observations, whose expression is, in general, intractable. Indeed, the likelihood is given by:
$$
\llh{n}(\parvec)  = \int \chi(x_0)\md{0;\parvec}(x_{0},Y_{0})\prod_{k=0}^{n-1}\qg{k;\parvec}(x_{k},x_{k+1}, Y_{k+1})\rmd x_{0:n},
$$
...
\end{equation*}
Expectation Maximization based algorithms \cite{dempster1977maximum}  are appealing solutions to obtain an estimator of $\hat \parvec$.
The pivotal concept of the EM algorithm is that the intermediate quantity defined by
\begin{equation}
\parvec\mapsto Q(\parvec,\parvec') = \pE_{\parvec'}\left[\sum_{k=0}^{n-1} \log \qg{k;\parvec}(X_{k},X_{k+1}, Y_{k + 1})\middle | Y_{0:n}\right] \label{eq:EM:E:step}
\end{equation}
...
Again, this inference setting is a special case of our framework where $\addf{k}(x_k, x_{k+1}) = \log \qg{k;\parvec}(X_{k},X_{k+1})$ (the dependance in $Y_{k +1}$ is omitted as the expectation is computed conditionnally to the observations).
