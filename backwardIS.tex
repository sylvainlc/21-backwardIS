...
where $\deriv \logllh{\parvec}(Y_k \mid Y_{1:k - 1})$ is the log likelihood for the new observation given all the past, and $\gamma_k$ is step size controlling the magnitude of the gradient ascent. 
\end{itemize}
...
where 
$$\af{0:k}(x_0:k} =  \sum_{j = 0}^{k - 1} \deriv\log \md{j+1;\parvec}(x_{j+1},Y_{j+1}) + \deriv \log \hd{j;\parvec}(x_j,x_{j+1})\eqsp.$$
