...
\textcolor{red}{Ne pas orienter vers les EDS uniquement. Mettre en avant les modeles complexes et l'impossibilite de faire de l'acceptation rejet, et mettre en avant la possibilite d'avoir un biais dans les poids. Les EDS seront une application parmi d'autres. Mettre en avant les differents points : utilisation d'estimateur des poids, importance sampling au lieu de l'AR pour un enorme gain numerique, application au gradient pour l'estimation en ligne.}
 ...
 In this setting, performing maximum likehood estimation (MLE) for instance with the Expectation Maximization (EM) algorithm  \cite{dempster1977maximum} or a stochastic gradient ascent (\cite{cappe2005inference} in the case of HMMs) is a challenging task.  Both approaches involve conditional distributions of sequences of hidden states given the observation record (the \textit{smoothing} distribution), which are not available explicitly.  
 Markov chain Monte Carlo (MCMC) and sequential Monte Carlo (SMC) methods (also known as particle filters or smoothers) are widespread solutions to propose consistent estimators of such distributions. However, a pivotal tool of both MCMC and SMC approaches is the evaluation of the density of the conditional distribution of $X_{t_k}$ given $X_{t_{k-1}}$ for all $1\leqslant k \leqslant n$. 
...
%The extension of the  usual alternative, named the Forward Filtering Backward Smoothing algorithm \cite{doucet2000sequential}, is more sensitive as it involves ratios of these unknown quantities.
  Other smoothing algorithms such as two-filter based approaches \cite{briers2010smoothing, fearnhead2010sequential, nguyen2017two}  could be extended  but they are intrisically not online procedures as they require the time horizon and all observations to be available  to initialize a backward information filter. In \cite{gloaguen2018online, gloaguen2019pseudo}, the consistency of this algorithm as long as a central limit theorem (CLT) are established.
  This makes this pseudo marginal smoother the first algorithm to approximate such expectations in the general setting of this paper with theoretical guarantees and an explicit expression of the asymptotic variance. 
  This work extended the result of \cite{olsson2017efficient}, written only in the case of the bootstrap filter \cite{gordon1993novel}, and the theoretical guarantees obtained for online sequential Monte Carlo smoothers given in \cite{delmoral2010backward, douc2011sequential, dubarry2013nonasymptotic, gerber2017convergence}.


However, a requirement to use this algorithm is that the unbiased estimate of the transition is almost surely positive and upper bounded.  This condition is required to perform a pivotal backward  acceptance-rejection sampling procedure. 
...
Moreover, this method does not rely on any acceptance rejection procedure which reduces very significantly the computational time and its variance, as illustrated in the numerical experiments.


\section{Model and objectives}
\label{sec:model}
\textcolor{red}{Faire plus generique, pas uniquement les EDS.}
Let $\parspace\subset \mathbb{R}^q$ be a compact parameter space and $(X_t)_{t\geqslant 0}$ be defined as a weak solution to the following 
SDE in $\mathbb{R}^d$:
...
It is assumed that the solution to \eqref{eq:sde} is partially observed at times $t_0 = 0,\ldots,t_n$, for a given $n\geqslant 1$, through an observation process $(Y_k)_{0\leqslant k \leqslant n}$ taking values in $\mathbb{R}^m$. 
For all $0\leqslant k \leqslant n$, the distribution of $Y_k$ given $(X_t)_{t\geqslant 0}$ depends on $X_k = X_{t_k}$ only and has density $\md{k;\parvec_*}$ with respect to the Lebesgue measure. 
The distribution of $X_0$ has density $\chi$ with respect to the Lebesgue measure and for all $0\leqslant k \leqslant n-1$, the conditional distribution of $X_{k+1} $ given $(X_{t})_{0\leqslant t\leqslant t_k}$ has density $\hd{k+1;\parvec_*}(X_{k},\cdot)$. 
...
In the context of HMMs, the gradient of the log-likelihood can also be expressed  as an expectation of an additive functional of the hidden states given $Y_{0:n}$ (\cite{cappe2005inference}, Chapter~10, or \cite{gloaguen2019pseudo} in the context of SDEs).

A key feature here is that all the relevant estimators rely on computing, for some parameters $\parvec$ and $\parvec'$:
...
%For all $p\geqslant 0$, the intermediate quantity is a conditional expectation of an additive functional and writes
%\[
%Q(\parvec,\parvec_{p}) = \post{0:n \mid n;\parvec_p}[\af{0:n;\parvec}] \eqsp,%= \mathbb{E}_{\parvec_p}\left[\sum_{k=0}^{n-1} \log \qg{k;\parvec}(X_{k},X_{k+1})\middle | Y_{0:n}\right] 
%\]
%where 
%$$
%\af{0:n,\parvec}: x_{0:n} \mapsto \sum_{k=0}^{n-1}\addf{k;\parvec}(x_{k},x_{k+1})
...
%and 
%$$
%\addf{k;\parvec}: (x_{k:k+1}) \mapsto \log \qg{k;\parvec}(x_{k},x_{k+1})
%$$
...
%This expression of the EM intermediate quantity supports the keen interest for efficient algorithms to estimate conditional expectation of additive functionals  of the latent states given the observations. 

In the following, $\parvec$ is dropped from the notations for better clarity when there is no possible confusion. As noted for instance in \cite{cappe2005inference}, although the objective is to  obtain approximation of smoothing distributions, the filtering distribution is crucial as, for additive functionals,
\begin{align}
\post{0:n \mid n}[\af{0:n}] &= \post{n}\left[\tstat{n}[\af{0:n}]\right] \nonumber,
\intertext{where}
\tstat{n}[\af{0:n}](X_n) &= \pE_{}\left[\af{n}(X_{0:n})\middle | X_n, Y_{0:n}\right]. \label{eq:T:stat}
\end{align}
As a key consequence of the additive property, for all $1\leqslant k\leqslant n$
\begin{equation}
\tstat{k}[\af{0:k}](X_k) = \pE_{}\left[\tstat{k - 1}[\af{0:(k - 1)}](X_{k - 1}) + \addf{k-1}(X_{k-1}, X_k)\middle | X_k, Y_{0:{k - 1}}\right]. \label{eq:T:add:recursion}
\end{equation}
However, the exact computation of all these key expectations is not possible in general state spaces. 

The next section describes a Sequential Monte Carlo algorithm \cite{doucet2013sequential} to approximate $\post{n}$ by weighted samples $\{(\ewght{n}{\ell},\epart{n}{\ell})\}_{\ell=1}^{\N}$ and the algorithm of \cite{gloaguen2018online} to compute recursively, for each $1\leqslant \ell \leqslant N$ an approximation $\tstat[\ell]{n}$ of $\tstat{n}[\af{0:n}](\epart{n}{\ell})$ so that the estimator of $\post{0:n \mid n}[\af{0:n}]$ is defined as
\begin{align}
\label{eq:paris:estimator}
\post{0:n \mid n}^N[\af{0:n}]&\eqdef \sum_{\ell=1}^N \frac{\ewght{n}{\ell}}{\sum_{j=1}^N \ewght{n}{j}}\tstat[\ell]{n}\eqsp.
\end{align}
...
\textcolor{red}{Ajouter le cas biase ou les poids sont estimes mais avec un biais, typiquement estimateurs a noyaux, cf papier pseudo marginal}
In the case of POD processes, SMC methods cannot be used straightforwardly as the transition densities $\hd{k}$, $0\leqslant k \leqslant n-1$, are unknown. 
To overcome these issues, following \cite{fearnhead2008particle, olsson2011particle, gloaguen2018online}, consider the following assumption. 
%Let $(\marginalset,\mathcal{B}(\marginalset))$ be a general state space.
\begin{hypH} 
...
is a consistent estimator of $\post{0}[f]$. Then, for all $k\geqslant 1$, once the observation $Y_k$ is available, the weighted particle sample $\{(\ewght{k-1}{\ell},\epart{k-1}{\ell})\}_{\ell=1}^{\N}$ is transformed into a new weighted particle sample approximating $\post{k}$.
 This update step is carried through in two steps, \emph{selection} and \emph{mutation},  using sequential importance sampling and resampling steps. % the auxiliary sampler introduced in \cite{pitt1999filtering}.  
...
\label{sec:AR}
In the context of additive functionals, the forward-only smoothing algorithm introduced in \cite{delmoral2010backward} proposes a particle approximation of \eqref{eq:T:stat} that can be computed \textit{online} using the recursion \eqref{eq:T:add:recursion}. 
This algorithm has a computational complexity which  grows \emph{quadratically} with the number of particles $\N$. 
This computational cost can be reduced when the transition density of the hidden states is upper bounded following \cite{olsson2017efficient}  by applying the accept-reject sampling approach proposed in~\cite{douc2011sequential} and illustrated in \cite{dubarry2011fast}. 
...
\section{Backward importance sampling for PODs}



\subsection{Positive parametrix estimates}
\textcolor{red}{Parler d'estimateur positifs au sens large, pas necessairement parametrix et pour les EDS. }
In this section, we propose to use Wald’s identity for martingales to obtain a new estimator based on the parametrix approach, which is guaranteed to be positive.  This identity was applied in \cite{fearnhead2010random} to Poisson based estimators. Our estimator is defined up to an unknown constant of proportionality, which is removed when the importance weights are normalized in equation \eqref{eq:update:paris:marginal:is}. 
This approach, rather than setting negative weights to 0, which would lead to a biased estimate, uses extra simulation to obtain positiveness. This is done while ensuring that the weights remain unbiased up to a common constant of proportionality. Assume that the distribution $\kernelmarg_{k}$ of the additional random variables $\zeta_k$ and the estimator $\hatqg{k}$ are obtained with the parametrix estimator.
...
\item While there exists $i_* \in\{1,\ldots,\N\}$ such that $\ewght{k}{i_*} \leqslant 0$, for all $1\leqslant i\leqslant \N$, sample $\zeta_k^{i}$ with distribution $\kernelmarg_{k}(\epart{k-1}{I_k^{i}},\epart{k}{i};\cdot)$ (i.e. compute a parametrix estimator of the transition density) and set 
$$
...
\textcolor{red}{Ici on met les resultats numeriques sur les esperances + estimation d'un RNN avec formule de fisher et approximation du gradient par la formule ci-dessus.}


\section{Tangent filters and online recursive maximum likelihood}
\label{sec:tangent:filter}
Let $\parspace$ be a parameter space. This section considers a family of transition kernels $(\hk_{k;\parvec})_{\parvec\in\parspace; 0\leqslant k \leqslant n-1}$ on $\Xset\times \mathcal{B}(\Xset)$ and $(\mk_{k;\parvec})_{\parvec\in\parspace;1\leqslant k \leqslant n}$ on $\Xset\times \mathcal{B}(\Yset)$ associated with densities $\hd{k;\parvec}$ and $\md{k;\parvec}$ with respect to $\mu$ and  $\nu$. The joint smoothing distributions are then defined, for any $\parvec\in\parspace$, $0 \leqslant k_1 \leqslant k_2 \leqslant n$ and any function $h\in \bmf{\Xset^{k_2 - k_1 +1}}$,  by:
\[
...
\textcolor{red}{Experiences a remonter dans la section sur les esperances}
...
\]
\subsubsection{General Poisson Estimators}

In \cite{olsson2011particle}, \cite{gloaguen2018online} and \cite{gloaguen2019pseudo}, General Poisson Estimators (GPEs) are used to obtain an unbiased estimate of the transition density.  However, designing such estimators requires three strong assumptions \cite{beskos2006retrospective}.
...
\subsubsection{Parametrix estimators}
...
%\paragraph{Continuous importance sampling estimators}
%The range of applicability of General Poisson estimators is limited by (a) the possibility to reduce the SDE to a SDE with unit diffusion matrix and (b) the requirement that the drift of the reduced SDE is the gradient of a potential function. 
%More recently, \cite{fearnhead:latuszynski:roberts:sermaidis:2017} developed a continuous-time importance sampling (CIS) estimator of the transition density that does not require such strong assumptions. This estimator is designed in a similar fashion as a discrete-time importance sampling estimator and does not rely on an acceptance-rejection mechanism on the path-space of the SDE. 

%We describe here the random variable $\zeta$ and the estimator $\widehat{q}_{1,\parvec}(x,y;\zeta)$ of $q_{1,\parvec}(x,y)$.


%Let $\mathcal{K}_{\parvec}$ be the forward operator of the process solution to the SDE \eqref{eq:sde}: for all bounded function $h$ on $\mathbb{R}^d$,
%\[
%\mathcal{K}_{\parvec}h: y\mapsto -\sum_{i=1}^d \frac{\partial (\alpha_{\parvec,i}h)}{\partial y_i}(y) + \sum_{i=1}^d\sum_{j=1}^d \frac{\partial (\gamma_{\parvec,ij}h)}{\partial y_i\partial y_j}(y)\eqsp,
%\]
%where $\gamma_{\parvec} = \Sigma_{\parvec}\Sigma_{\parvec}'$. The CIS algorithm draws weighted skeletons at random times $(\tau_1,\dots,\tau_p)$, $(x_{\tau_i}(\parvec),\omega^{\mathsf{CIS}}_{\tau_i}(\parvec))_{0\leqslant i \leqslant\tau_{p}}$, where $\omega^{\mathsf{CIS}}_0(\parvec)=1$, $\tau_0=0$. The update times $(\tau_1,\dots,\tau_p)$ are instances of a Renewal process with intensity $\lambda$ and $\tau_{p}$ is the last event before $t_1$ and denoted $\tau_{t_1}$. Let $0\leqslant k \leqslant p-1$, $(x_{\tau_k}(\parvec),\omega^{\mathsf{CIS}}_{\tau_k}(\parvec))$ be the last weighted sample and $\tau_{k+1}$ be the next update time of the trajectory. The new state is sampled using
%\begin{align*}
%x_{\tau_{k+1}}(\parvec) &\eqdef x_{\tau_k}(\parvec) + \Delta\tau_k\alpha_{\parvec}(x_{\tau_k}(\parvec)) + \Delta\tau_k^{1/2}\Sigma_{\parvec}(x_{\tau_k}(\parvec))\xi_{k+1}\eqsp,
%\end{align*}
%where $\Delta\tau_k \eqdef \tau_{k+1}-\tau_k$ and $\xi_{k+1}\sim\mathcal{N}_d(0,I_d)$. The proposal density associated with this procedure is denoted by $q\left(x_{\tau_k}(\parvec),\cdot,\Delta\tau_k,\parvec\right)$. For any $x,y\in\mathbb{R}^d$, $u\in[0,T]$ and any $\parvec\in\parvec$,
%\begin{equation}
%\label{CIS:eq:prop}
%q\left(x,y,u,\parvec\right) \eqdef \frac{\left(2\pi\right)^{-d/2}}{\sqrt{\mathrm{det}\left(u\gamma_{\parvec}(x)\right)}}\mathrm{exp}\left\{-\frac{1}{2u}\left(y-x-\alpha_{\parvec}(x)u\right)^{t}\gamma_{\parvec}^{-1}(x)\left(y-x-\alpha_{\parvec}(x)u\right)\right\}\eqsp.
%\end{equation}
%Let $\mathcal{K}^{k,\parvec}_{\mathrm{prop}}$ denotes the Kolmogorov forward operator of the proposal distribution $q\left(x_{\tau_k}(\parvec),\cdot,\Delta\tau_k,\parvec\right)$. The forward operators write, for any function $h:\mathbb{R}^d\rightarrow\mathbb{R}$,
%\[
%\mathcal{K}^{k,\parvec}_{\mathrm{prop}}h\left(y\right) \eqdef -\sum_{i=1}^d\alpha_{\parvec,i}(x_{\tau_k}(\parvec))\frac{\partial h}{\partial y_i}\left(y\right) +\sum_{i,j=1}^d\frac{\gamma_{\parvec,i,j}(x_{\tau_k}(\parvec))}{2}\frac{\partial^2h}{\partial y_i\partial y_j}\left(y\right)\eqsp.
%\]
%Then, following \cite{fearnhead:latuszynski:roberts:sermaidis:2017}, the weight is updated by
%\[
%\omega^{\mathsf{CIS}}_{\tau_{k+1}}(\parvec)\eqdef\omega^{\mathsf{CIS}}_{\tau_k}(\parvec)\rho^{\lambda}_k\left(x_{\tau_k}(\parvec),x_{\tau_{k+1}}(\parvec),\Delta\tau_k,\parvec\right)\eqsp,
%\]
%where
%\begin{equation}
%\label{CIS:eq:rho}
%\rho^{\lambda}_k\left(x,y,u,\parvec\right)\eqdef 1+\frac{\left(\mathcal{K}-\mathcal{K}^{k,\parvec}_{\mathrm{prop}}\right)q\left(x,z,u,\parvec\right)_{|z=y}}{\lambda(u)q\left(x,y,u,\parvec\right)}\eqsp.
%\end{equation}
%
%\begin{algorithm}[h!]
%\caption{CIS of \cite{fearnhead:latuszynski:roberts:sermaidis:2017}}
%\label{CIS:alg:continuous}
%\begin{algorithmic}[1]
%\REQUIRE $t_1$, $\lambda$.
%\STATE Set $\tau_0=0$, $x_0(\parvec)=x$, $\omega^{\mathsf{CIS}}_0(\parvec)=1$, $k=0$.
%\WHILE{$\tau_k<t_1$}
%\STATE Draw $u$ the interarrival time of a renewal process of rate $\lambda$.  
%\STATE Set $\tau_{k+1}=\tau_k +u$.
%\STATE Draw $x_{\tau_{k+1}}(\parvec) \sim q\left(x_{\tau_k}(\parvec),\cdot,\Delta\tau_k,\parvec\right)$, where $q$ is defined by \eqref{CIS:eq:prop}.
%\STATE Set $\omega^{\mathsf{CIS}}_{\tau_{k+1}}(\parvec)=\omega^{\mathsf{CIS}}_{\tau_k}(\parvec)\rho^{\lambda}_k\left(x_{\tau_k}(\parvec),x_{\tau_{k+1}}(\parvec),\Delta\tau_k,\parvec\right)$, where $\rho$ is defined by \eqref{CIS:eq:rho}.
%\STATE Set $k = k+1$.
%\ENDWHILE
%\end{algorithmic}
%\end{algorithm}
%If Algorithm~\ref{CIS:alg:continuous} is repeated independently $M$ times and produces $(x_{\tau^i_{t_1}}^i(\parvec),\omega_{\tau^i_{t_1}}^{\mathsf{CIS},i}(\parvec))_{1\leqslant i \leqslant M}$ and $\zeta$ gathers all the random variables sampled by CIS, the estimator of $q_{1,\parvec}(x,y)$ is given by:
%\[
%\widehat{q}_{1,\parvec}(x,y;\zeta) =  \frac{1}{M}\sum_{i=1}^M\omega_{\tau^i_{t_1}}^{\mathsf{CIS},i}q\left(x^i_{\tau^i_{t_1}}(\parvec),y,t_1-\tau^i_{t_1},\parvec\right)\eqsp.
%\]
%The stability and unbiasedness of this estimator is studied in \cite{fearnhead:latuszynski:roberts:sermaidis:2017}. 
