...
In this framework, the likelihood of the observations $\llh{n,\parvec}(Y_{0:n})$ , whose expression is, in general, intractable, is given by:
$$
...
\end{equation}
In some models, this quantity might be unknown (see models of sections \ref{sec:simu:SINE} and \ref{sec:simu:LV}), and we will only suppose that we have an estimate of this quantity (see assumption \textbf{H\ref{assum:unbiased}} in section \ref{sec:method}).
Note that to avoid future cumbersome expressions,the dependance of the key quantity $\qg{k;\parvec}(\cdot)$ on the observations is implicit (as we will always work conditionnaly to the observations).
...
\end{equation}
where $\addf{k}$ is a functional from $\rset^{d \times 2}$ to $\rset^{d'}$.
...
An extension of this method will be illustrated on section \ref{sec:tangent:filter} on an original example where the transition density cannot be evaluated.
\end{example}
...
In \cite{gloaguen2018online} and \cite{gloaguen2019pseudo}, the authors assumes that for all  $0\leqslant k\leqslant n$ and $0\leqslant i\leqslant N$, there exists an upper bound $\hkup^i_k$  such that
\begin{equation}
\mathrm{sup}_{\ell,\zeta}\;\hatqg{k}\langle \zeta\rangle(\epart{k}{\ell},\epart{k+1}{i})\leqslant \hkup^i_k\eqsp. \label{eq:AR:bound}
\end{equation}
Approximation of the backward statistics (as defined in \eqref{eq:T:stat}) are compute recursively, for each simulated particles. 
...
$$
The sampling from this specific distribution is possible thanks to condition \eqref{eq:AR:bound}, as, for all $(i, z) \in \lbrace 1,\dots,N\rbrace\times\marginalset$, it results in
$$
...
However, it relies on the key assumption \eqref{eq:AR:bound} which is rather restrictive, and would not be satisfied in practice for a lot of problems (as we will see in section \ref{sec:simu:LV}).
