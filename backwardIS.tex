...
From this general state space model based on a stochastic RNN, in the context of the \textit{state estimation} problem, we can use particle smoothing algorithms to estimate the smoothing expectation $\mathbb{E}[X_k|Y_{1:n}]$. In practice, we train a deterministic one-layer RNN on 20,000 samples of a real-world time-series dataset, the weather dataset\footnote{https://www.bgc-jena.mpg.de/wetter/}, and inject noise post-training on the trained RNN to generate sequence of states and observations $(X_{0:n},Y_{0:n})$.  We consider diagonal covariance matrices $\Sigma$, $Q$ and $R$, with the same variance along all dimensions equal to 0.1. 

Table~\ref{table:RNN_exp} presents the Mean Squared error $\|X_0 - \mathbb{E}[X_0|Y_{1:n}]\|^2$ for the backward IS and the Poor's man smoothers, for RNNs of hidden dimension 32 and 64, when considering sequences of observations of respective size $n=24$ and $n=100$, and various numbers of backward samples $\K$. For a similar computational cost, the Backward IS smoother outperforms the Poor man's smoother in every configuration, with a smaller Mean Squared Error.  
...
\begin{table}
\caption{Mean squared error $\|X_0 - \mathbb{E}[X_0|Y_{0:n}]\|^2$ for the Poor man's smoother (PMS) and the Backward IS smoother with 1000 particles, for states $\{X_k\}_{k=0}^n$ and observations $\{X_k\}_{k=0}^n$ generated with stochastic RNNs of dimension 32 and 64, for various number $\K$ of backward samples. We consider sequences of observations with $n=24$ and $n=100$.}
\label{table:RNN_exp}
\centering 
...
\hline
$d = 32$ & $\K = 2$ & 0.2601 & 0.2434 & 0.2196 & 0.1812 \\
$d = 32$ & $\K = 4$ & X & 0.2494 & X & 0.1877 \\
$d = 32$ & $\K = 8$ & X & 0.2541 & X &  \\
\hline
$d = 64$ & $\K = 2$ & 0.1498 & 0.1473 & 0.2176 & 0.1909 \\
$d = 64$ & $\K = 4$ & X & & X & \\
$d = 64$ & $\K = 8$ & X & & X & \\
\hline
\bottomrule
