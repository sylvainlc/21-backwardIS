...
However, as with the PaRIS algorihm,  when using this pseudo-marginal approach, where transition densities are intractable, the user needs to be able to sample exactly from the associated pseudo-marginal backward kernel. 
...
In this paper, a new procedure is introduced to replace the backward acceptance-rejection step of the PaRIS and the pseudo marginal PaRIIS algorithms by a backward importance sampling estimate.  %which 
...
In the general case where only signed estimates can be obtained, we propose to use of the Wald trick, ensuring positiveness without addition of bias in the estimator.
In the context of partially observed diffusion processes, for instance,  we show that combining this Wald trick to the  parametrix estimators of \cite{andersson2017unbiased} and \cite{fearnhead2017continuous} leads to an highly generic algorithm that can be applied to a wide class of models, for which no low variance smoother existed so far.
