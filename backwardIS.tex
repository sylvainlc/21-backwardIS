...
In this section, we focus in the context of online recursive maximum likelihood, i.e., where new observations are used only once to update our estimator of the unknown $\parvec$.
...
Following \cite{legland1997recursive}, the idea is the build a sequence $\left\lbrace\parvec_k\right\rbrace_{k\geq 0}$ as follows.

\begin{itemize}
\item Using the first observation $Y_0$, initialize $\parvec_0$;
\item For a new observation $Y_{k},~k\geqslant 1$, set:
$$\theta_{k} = \theta_{k-1} + \gamma_k \deriv \logllh{\parvec}(Y_k \mid Y_{1:k - 1}) ,$$
where $\deriv \logllh{\parvec}(Y_k \mid Y_{1:k - 1})$ is the log likelihood for the new observation given all the past, and $\left\lbrace\gamma_k\right\rbrace_{k\geqslant}$ is positive step size such that $\sum_{k \geqslant 1}\gamma_k = \infty$ and $\sum_{k \geqslant 1}\gamma_k^2 < \infty$. 
\end{itemize}

One can verify that, in the context of our model, we have: 
\begin{equation}
...
where $\pred{k;\parvec} = \phi_{k \mid k - 1}$ is the predictive distribution, i.e. the law of $X_k\vert Y_{0:{k-1}}$, and 
$$\filtderiv{k;\parvec}[\md{k;\parvec}] = \int \md{k;\parvec}(x_k) \deriv \pred{k;\parvec}(x_k) \rmd x_k,$$
is known as the \textit{tangent filter} (in this case, for the functional $\md{k,\parvec}$, see \cite{cappe2005inference}, Chapter 10, \cite{delmoral2015uniform} or \cite{olsson2020particle} for a more generic definition).
...
\end{equation}
Therefore, a particle filter, can be used to compute 
$$\pred{k}^N[\md{k;\parvec}] = \frac{1}{N}\sum_{\ell = 1}^N \md{k;\parvec}(\epart{n}{\ell})\eqsp,\eqsp \pred{k}^N[\deriv \md{k;\parvec}] = \frac{1}{N}\sum_{\ell = 1}^N \deriv \md{k;\parvec}(\epart{k}{\ell}),$$
together with a backward sampling procedure, approximating the backward statistic related to the functional \eqref{eq:complete:gradient:log}, resulting in the approximation
\begin{equation} 
...
For the interested reader, it might be worth noting the idea behind this online algorithm. 
First, note that 
$$
